...
This project provides a Docker container for [Krokiet](https://github.com/qarmin/czkawka).
...
services:
  krokiet:
    image: jlesage/krokiet
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/krokiet:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-krokiet/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/krokiet/tags
...
container, it would proxy all HTTP requests for `krokiet.domain.tld` to
...
server {
	...
	server_name krokiet.domain.tld;

	location / {
		proxy_pass http://docker-krokiet;
...
server {
	...
	location = /krokiet {return 301 $scheme://$http_host/krokiet/;}
	location /krokiet/ {
		proxy_pass http://docker-krokiet/;
		proxy_http_version 1.1;
		proxy_set_header Upgrade $http_upgrade;
...
| **Application version** label | Displays the version of Krokiet integrated into Docker image. |
...
Any desktop notification generated by Krokiet is forwarded to the
...
When trash usage is enabled in Krokiet settings, deleted files are
